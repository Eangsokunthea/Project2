...
class BrandProduct extends Controller
{
    ...
    public function all_brand_product(){
        ...
        $all_brand_product = Brand::orderBy('brand_id','DESC')->get();// co the su dung pagination o day!!! 
    ...
    public function save_brand_product(Request $request){
    	...
    	return Redirect::to('add-brand-product')->with('message', 'Thêm thương hiệu sản phẩm thành công');
    }
    public function unactive_brand_product($brand_product_id){
        $this->AuthLogin();
        DB::table('tbl_brand')->where('brand_id',$brand_product_id)->update(['brand_status'=>1]);
        return Redirect::to('all-brand-product')->with('message', 'Không kích hoạt thương hiệu sản phẩm thành công');

    }
    public function active_brand_product($brand_product_id){
        $this->AuthLogin();
        DB::table('tbl_brand')->where('brand_id',$brand_product_id)->update(['brand_status'=>0]);
        return Redirect::to('all-brand-product')->with('message', 'Kích hoạt thương hiệu sản phẩm thành công');
    ...
    public function update_brand_product(Request $request,$brand_product_id){
        ...
        return Redirect::to('all-brand-product')->with('message', 'Cập nhật thương hiệu sản phẩm thành công');
    }
    public function delete_brand_product($brand_product_id){
        $this->AuthLogin();
        DB::table('tbl_brand')->where('brand_id',$brand_product_id)->delete();
        return Redirect::to('all-brand-product')->with('message', 'Xóa thương hiệu sản phẩm thành công');
    }
